...
public class KarafJMXConnector {
    ...
    public Long install(File bundleFile, Boolean start) throws InstanceNotFoundException, MBeanException, ReflectionException, IOException {
        return (Long) connection.invoke(karafBundles, "install",
                new Object[]{String.format("file://%s", bundleFile.getAbsolutePath()), start},
                new String[]{"java.lang.String", boolean.class.getName()});
    }
    ...
    private void init() throws MalformedURLException {
        ...
        await().atMost(timeout, TimeUnit.MILLISECONDS).pollInterval(1, TimeUnit.SECONDS).until((Callable<Boolean>) () -> {
            try {
                ...
                return !connection.getObjectInstance(karafBundles).getClassName().isEmpty()
